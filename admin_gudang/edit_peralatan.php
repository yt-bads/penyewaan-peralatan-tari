...
// Pastikan ID peralatan ada dan valid
...
// Ambil data peralatan existing
try {
    $stmt = $pdo->prepare("SELECT * FROM peralatan WHERE id = ?");
    $stmt->execute([$id]);
    $peralatan = $stmt->fetch();
    
    if (!$peralatan) {
        $_SESSION['error'] = 'Data peralatan tidak ditemukan.';
        header('Location: daftar_peralatan.php');
        exit();
    }
} catch (PDOException $e) {
    $_SESSION['error'] = 'Gagal memuat data karena kesalahan sistem.';
    header('Location: daftar_peralatan.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nama_alat === '' || $kategori_alat === '' || $stok === '' || $harga_sewa === '' || $kondisi === '') {
        ...
        header("Location: edit_peralatan.php?id={$id}");
        exit();
    }
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] !== UPLOAD_ERR_NO_FILE) {
        ...
        if (move_uploaded_file($file_tmp, $target_dir . $nama_foto)) {
            ...
            if (!empty($peralatan['foto'])) {
                $old_file = $target_dir . $peralatan['foto'];
    ...
    try {
        if ($upload_baru) {
            $stmtUpdate = $pdo->prepare("
                UPDATE peralatan 
                SET nama_alat = ?, kategori_alat = ?, stok = ?, harga_sewa = ?, kondisi = ?, deskripsi = ?, foto = ?
                WHERE id = ?
            ");
            $stmtUpdate->execute([
                $nama_alat,
            ...
            $stmtUpdate = $pdo->prepare("
                UPDATE peralatan 
                SET nama_alat = ?, kategori_alat = ?, stok = ?, harga_sewa = ?, kondisi = ?, deskripsi = ?
                WHERE id = ?
            ");
            $stmtUpdate->execute([
                $nama_alat,
        ...
        $_SESSION['success'] = "Peralatan '{$nama_alat}' berhasil diperbarui!";
        header('Location: daftar_peralatan.php');
        exit();

    } catch (PDOException $e) {
        $_SESSION['error'] = 'Gagal menyimpan perubahan karena kesalahan sistem: ' . $e->getMessage();
        header("Location: edit_peralatan.php?id={$id}");
        exit();
    }
}
...
<head>
    ...
    <title>Edit Peralatan - Admin Gudang</title>
...
<body>
    ...
    <div class="dashboard-container">
        <div class="dashboard-header" style="max-width: 650px; margin: 0 auto 2rem auto;">
            ...
            <h1 style="margin-top: 0.5rem;">Edit Peralatan</h1>
            <p>Ubah detail informasi perlengkapan tari sanggar.</p>
        </div>
        ...
        <div class="form-card">
            <form action="edit_peralatan.php?id=<?= $id ?>" method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    <label for="nama_alat" class="form-label">Nama Peralatan</label>
                    <input type="text" name="nama_alat" id="nama_alat" class="form-control" placeholder="Contoh: Kostum Tari Pendet Bali" value="<?= htmlspecialchars($peralatan['nama_alat']) ?>" required autofocus>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="kategori_alat" id="kategori_alat" class="form-control" required>
                            ...
                            <option value="kostum" <?= $peralatan['kategori_alat'] === 'kostum' ? 'selected' : '' ?>>Kostum</option>
                            <option value="aksesoris" <?= $peralatan['kategori_alat'] === 'aksesoris' ? 'selected' : '' ?>>Aksesoris</option>
                            <option value="alat_musik" <?= $peralatan['kategori_alat'] === 'alat_musik' ? 'selected' : '' ?>>Alat Musik</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="kondisi" id="kondisi" class="form-control" required>
                            ...
                            <option value="perlu_perbaikan" <?= $peralatan['kondisi'] === 'perlu_perbaikan' ? 'selected' : '' ?>>Perlu Perbaikan</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" name="stok" id="stok" class="form-control" min="0" placeholder="0" value="<?= htmlspecialchars($peralatan['stok']) ?>" required>
                    </div>

                    <div class="form-group">
                        <label for="harga_sewa" class="form-label">Harga Sewa (Rp)</label>
                        <input type="number" name="harga_sewa" id="harga_sewa" class="form-control" min="1000" max="10000000" step="500" placeholder="Nominal tanpa tanda titik" value="<?= htmlspecialchars((int)$peralatan['harga_sewa']) ?>" required>
                ...
                <div class="form-group">
                    ...
                    <div style="margin-bottom: 1rem;">
                        <?php if (!empty($peralatan['foto'])): ?>
                            <img src="../uploads/foto_peralatan/<?= htmlspecialchars($peralatan['foto']) ?>" style="max-height:150px; display:block; margin-bottom:0.5rem; border-radius: var(--radius-sm); border: 1px solid var(--border);" alt="Foto saat ini">
                            <p style="font-size: 0.85rem; color: var(--text-muted); margin: 0.25rem 0;">Biarkan kosong jika tidak ingin mengganti foto.</p>
                        <?php else: ?>
                            <p style="font-size: 0.85rem; color: var(--text-muted); margin: 0.25rem 0;">Belum ada foto.</p>
                        <?php endif; ?>
                    </div>
                    <label for="foto" class="form-label">Pilih Foto Baru (Opsional)</label>
                ...
                <div class="form-group">
                    ...
                    <textarea name="deskripsi" id="deskripsi" class="form-control" placeholder="Masukkan spesifikasi produk, kelengkapan, ukuran, atau panduan perawatan..."><?= htmlspecialchars($peralatan['deskripsi'] ?? '') ?></textarea>
                </div>
